...
一個純前端（Vanilla JS，無建置流程、無 npm）的繁體中文聊天機器人網頁，提供新生兒黃疸居家照護的衛教問答服務。使用者可透過 App 的 WebView 與後端 AI 客服對話，取得生理性／病理性黃疸、母乳哺餵、大便顏色觀察等衛教資訊。
...
- **多語言介面**：支援繁體中文／English／Indonesia／Tiếng Việt 四種語言，各語言有獨立歡迎訊息，並會隨每次請求送往後端。
...
- **危險關鍵字攔截**：偵測到使用者輸入包含高風險字詞（如「自殺」）時，立即鎖住輸入框與送出鍵，並跳出警示視窗（`#dangerAlertModal`），提醒使用者尋求專業協助。⚠️ 目前關鍵字清單僅供測試，尚待擴充。
...
- **WebView 橋接**：供原生 App（Flutter）注入 `babyId`、`parentId`、Firebase `authToken`，讓同一套網頁可同時服務一般訪客與已登入的 App 使用者。
- **行動裝置優先的 RWD**：處理 iOS 安全區域（`safe-area-inset`）、動態視窗高度（`dvh`）、鍵盤彈出等手機瀏覽情境。
- **內容安全政策（CSP）**：`index.html` 內建 CSP，限制腳本／樣式／連線來源，降低前端被注入攻擊的風險。
...
| `babyId` | `localStorage.babyID` | 由 App WebView 呼叫 `window.setBabyId(id)` 注入 |
| `parentId` | `localStorage.parentID` | 由 App WebView 呼叫 `window.setParentId(id)` 注入 |
| `authToken` | 僅存於記憶體 | 由 App WebView 呼叫 `window.setAuthToken(token)` 注入，重新整理後需重新注入 |

詳細的 WebView 注入時機與 Flutter 端範例程式碼，見 [`docs/flutter-webview-baby-id.md`](docs/flutter-webview-baby-id.md)。

### 語言切換

`#langSelect` 下拉選單對應 `WELCOME_MESSAGES` 中四種語言的歡迎詞。切換語言時會：
1. 將選擇寫入 `localStorage.fourleaf_language`
2. 清空目前的對話紀錄
3. 重新顯示對應語言的歡迎訊息
4. 之後每次請求的 `language` 欄位都會帶上目前語言
...
├── assets/                     # Logo／大頭貼圖片（大頭貼實際上未在執行時使用）
├── pic/                         # 對話內容中會用到的衛教圖片（大便顏色卡、膽道閉鎖示意圖）
├── docs/
│   └── flutter-webview-baby-id.md   # Flutter WebView 注入 babyId/parentId 的設計文件
├── .github/workflows/static.yml     # GitHub Pages 自動部署設定
└── CLAUDE.md                   # 給 Claude Code 的專案指引
```

> 注意：`assets/` 內的使用者／機器人大頭貼圖片實際執行時**不會被使用**——`app.js` 改為從 GitHub CDN（`raw.githubusercontent.com`）載入對應圖片，因此本機離線瀏覽時大頭貼會顯示不出來。
...
- **CSP**：`index.html` 設定了嚴格的 Content-Security-Policy，限制腳本僅能來自同源、連線僅能打到後端 API、圖片僅能來自同源與指定 CDN。
...
姊妹儲存庫 `jaundice_inside` 為面向醫護人員的變體版本（不同 API 端點、支援 `clinicianId`、無語言選單）。為獨立的 git 儲存庫／remote，非本儲存庫的分支，兩者變更不會自動同步。

---

## 更多文件

- [`CLAUDE.md`](CLAUDE.md) — 給 AI 協作工具（Claude Code）閱讀的專案架構說明
- [`docs/flutter-webview-baby-id.md`](docs/flutter-webview-baby-id.md) — Flutter App WebView 注入 `babyId`／`parentId` 的完整設計文件與範例程式碼
